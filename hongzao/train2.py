...
dpp = 0.5
#打印网络结构
print([mark,'n_dense=',n_dense,'learate=',learning_rate,'decay=',decay,decay2,'dropout=',dp,dpp,'regular=',regularate,'finetune=',133-fine_tune_at])
...
hiden_layer = Dense(n_dense,activation=None,kernel_regularizer=regularizers.l2(regularate/2))
...
print(model.summary())
#模型训练
early_stopping = tf.keras.callbacks.EarlyStopping(monitor='val_loss', min_delta=0.01, patience=2, verbose=1, mode='min',restore_best_weights=True)#,baseline=None
...
early_stoppingg = tf.keras.callbacks.EarlyStopping(monitor='val_accuracy', min_delta=0.005, patience=3, verbose=1, mode='max',restore_best_weights=True)
...
with open('F:/train_log.csv','a',newline='') as f:
	f_csv = csv.writer(f)
	f_csv.writerow([mark+'记录时间：',time,'运行时长:',ts+'s','n_dense=',n_dense,'learate=',learning_rate,'decay=',decay,decay2,'dropout=',dp,dpp,'regular=',regularate,'finetune=',133-fine_tune_at])
